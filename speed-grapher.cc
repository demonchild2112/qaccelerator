...
template<typename T>
string Join(const vector<T>& in, string del=" ") {
  stringstream ss;
  auto it = in.begin();
  while (it != in.end()) {
    ss << *it;
...
// TODO(ogaro): Look into std::enable_if for this. (The compiler gets confused
// when a const char array is passed in). Use this function for all dict
// lookups.
template<typename K, typename V>
const V& FindOrDie(const unordered_map<K, V>& dict_,
                         const K& key) {
  const auto& it = dict_.find(key);
  if (it == dict_.end()) {
    // TODO(ogaro): Actually die.
  }
  return it->second;
}
